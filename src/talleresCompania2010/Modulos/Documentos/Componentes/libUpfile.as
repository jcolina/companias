...
private var _modUpDoc:modUpDocumentos;
...
public function set objUpDocumentos(objUpDoc:modUpDocumentos):void{
	this._modUpDoc = objUpDoc;
}
...
private function onSelectFile(event:Event):void {
	...
	if (arrFoundList.length >= 1) {
		Alert.show(IdiomaApp.getText('up_file_documento')+" \n\n• " + arrFoundList.join("\n• ") + "\n\n"+IdiomaApp.getText('up_file_documento_mje'), IdiomaApp.getText('up_file_documento_cargado'));
	}
	updateProgBar();
	scrollFiles();
	uploadCheck();
...
private function startUpload():void {
	...
	if (_modUpDoc.validarDatos()){	
		if (_arrUploadFiles.length > 0) {
			...
			sendVars.title= validarDato(_modUpDoc.txTitulo.text);	
			sendVars.desc=validarDato(_modUpDoc.txaDescrip.text);;	
			...
			sendVars.pais = Application.application.Pais;
			sendVars.siniestro=objVeh.NumeroSiniestro;
			sendVars.patente=objVeh.Patente;	
			sendVars.tipoDocumento=_modUpDoc.cbCategoria.text;
			sendVars.etapa = _modUpDoc.cboEtapa.text;
...
private function onUploadComplete(event:Event):void {
	_numCurrentUpload++;	
		...
		_modUpDoc.cerrarDialog();
			}
}

// Called on upload io error
private function onUploadIoError(event:IOErrorEvent):void {
Alert.show(IdiomaApp.getText('up_file_alert_ya_existe'));
	clearUpload();
